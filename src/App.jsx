      ...
      <div className="grid grid-cols-2 gap-6">
        {sequence.map((pose, idx) => (
          <div key={pose.uniqueId} className="flex gap-4 p-4 border border-gray-200 rounded-xl break-inside-avoid">
            ...
            <div className="text-2xl font-mono font-bold text-gray-200 w-8 text-right">
              {idx + 1}
            </div>
          </div>
        ))}
    ...
    <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm animate-in fade-in duration-200" onClick={onClose}>
      <div className="bg-white dark:bg-stone-800 w-full max-w-2xl rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]" onClick={e => e.stopPropagation()}>
        <div className="h-64 bg-stone-100 dark:bg-stone-900 relative flex items-center justify-center overflow-hidden group">
          ...
          <button onClick={onClose} className="absolute top-4 right-4 p-2 bg-black/20 hover:bg-black/40 text-stone-700 dark:text-white rounded-full backdrop-blur-md"><X size={20} /></button>
        </div>
        <div className="p-5 md:p-8 overflow-y-auto">
          ...
          <div className="space-y-4">
             <div><h3 className="font-bold">Instructions</h3><p>{pose.cues}</p></div>
             <div><h3 className="font-bold">Benefits</h3><ul className="list-disc ml-5">{pose.benefits?.map((b,i) => <li key={i}>{b}</li>)}</ul></div>
          </div>
        </div>
      </div>
    </div>
  ...
  if (isTeacherMode) {
    return (
      <div className="flex items-center gap-4 p-3 border-b border-stone-200 dark:border-stone-700 break-inside-avoid hover:bg-stone-50 dark:hover:bg-stone-800/50 transition-colors">
        ...
        <div className="w-10 h-10 bg-stone-100 dark:bg-stone-800 rounded-full overflow-hidden shrink-0 flex items-center justify-center text-teal-600 dark:text-teal-400"><PoseIcon category={pose.category} className="w-6 h-6" /></div>
        <div className="flex-1">
      ...
      >
        <div className="flex justify-between items-start mb-2">
          <div className="flex items-center gap-4">
             <div className="w-14 h-14 bg-stone-50 dark:bg-stone-900 rounded-xl flex items-center justify-center text-teal-600 dark:text-teal-400 shrink-0"><PoseIcon category={pose.category} className="w-8 h-8" /></div>
             <div><h3 className="font-bold text-lg text-stone-800 dark:text-stone-100">{pose.name}</h3><p className="text-stone-500 dark:text-stone-400 italic text-sm">{pose.sanskrit}</p></div>
          </div>
